...
- 誤差のlog-logフィット結果を保存
- パウリ回転数/RZ・T-depth外挿
- 有限 RTE の列挙・標本平均、RPEラウンド打切り予算、dense-free DF-tailからQiskit event回路構築、単一event・短いevent列・DF partial-S2一反復および短い複数反復のcompiled期待コスト
...
反復APIは監査用provenance fingerprint（seed規約を含む）とcircuit
semantics fingerprint（seedを含まず実回路だけを表す）を分離する。transpile
cacheはさらに、数値bind済み実Qiskit回路の再帰serializerとbackend targetを
canonical hashして検証する。既定の
