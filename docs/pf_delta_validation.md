...
に加えて、論文Appendix DのEq. (D6)

$$
\Delta E_{\mathrm{pert,D6}}
=
\frac{
\operatorname{Re}\langle\psi_H|
[U_{\mathrm{partial}}(\delta)-e^{-iE_H\delta}]|\psi_H\rangle
}{\delta\sin(E_H\delta)}
$$

を主たる大規模系向け推定量として計算する。$|\sin(E_H\delta)|<0.1$の点は、
small-$\delta$補正式へ置換せずD6係数fitから除外する。
...
もsurvival phaseとの整合性を確認する診断量として保存するが、主たる
$C_{\mathrm{partial}}$には使わない。検証ではQiskit statevectorとsector行列積の一致、
D6と支配固有位相の差、およびそれぞれの二次則を確認する。
...
- full-$H$基底状態のEq. (D6)摂動係数が、同じwell-conditioned点でfitした
  支配固有位相係数と相対2%以内で一致
...
Eq. (D6)については、$|\sin(E_H\delta)|<0.1$となる$\delta=0.0125,0.025$を
各分割のD6 fitから除外し、残る4点を用いた。D6係数と同じ4点でfitした支配固有位相
係数の相対差は最大0.288%、well-conditioned pointごとの差は最大0.882%で、全12分割が
事前の2%条件を通過した。

| $L_D$ | $\lambda_R$ | $C_D$ | $C_{\mathrm{D6}}$ | $C_{\mathrm{peak}}^{\mathrm{same\ 4pt}}$ | $C_{\mathrm{RMSE}}$ | $C_{p}\;(p_{\mathrm{off}}\simeq C_p\delta^4)$ | 1.2倍$C_D$ |
|---:|---:|---:|---:|---:|---:|---:|:---:|
| 0 | 13.7237 | 0 | 0.01153377 | 0.01150069 | 0.03878915 | 0.000729833 | 不通過 |
| 1 | 2.862957 | 0.00997435 | 0.01344960 | 0.01341561 | 0.04036042 | 0.000756257 | 不通過 |
| 2 | 1.603096 | 0.01032642 | 0.01341822 | 0.01338419 | 0.04035252 | 0.000757495 | 不通過 |
| 3 | 0.583270 | 0.01163949 | 0.01339914 | 0.01336510 | 0.04034470 | 0.000757307 | 通過 |
| 4 | 0.0871678 | 0.01348773 | 0.01342539 | 0.01339128 | 0.04040358 | 0.000758892 | 通過 |
| 5 | 0.0544047 | 0.01348263 | 0.01342565 | 0.01339154 | 0.04040364 | 0.000758893 | 通過 |
| 6 | 0.0220789 | 0.01309744 | 0.01342567 | 0.01339156 | 0.04040362 | 0.000758891 | 通過 |
| 7 | 0.000312954 | 0.01336928 | 0.01342567 | 0.01339156 | 0.04040362 | 0.000758891 | 通過 |
| 8 | $8.33\times10^{-5}$ | 0.01336901 | 0.01342567 | 0.01339156 | 0.04040362 | 0.000758892 | 通過 |
| 9 | $1.56\times10^{-7}$ | 0.01336958 | 0.01342567 | 0.01339156 | 0.04040362 | 0.000758892 | 通過 |
| 10 | 0 | 0.01336958 | 0.01342567 | 0.01339156 | 0.04040362 | 0.000758891 | 通過 |
| 11 | 0 | 0.01336958 | 0.01342567 | 0.01339156 | 0.04040362 | 0.000758891 | 通過 |
...
今回の小規模基準では単一の支配固有位相を仮定し、PF係数として

$$
C_{\mathrm{PF,eig}}=C_{\mathrm{peak}}\simeq0.01339
$$

を使う。大規模系での推定器にはfull-$H$基底状態に対するEq. (D6)摂動係数
$C_{\mathrm{partial,D6}}\simeq0.01343$を使う。同じwell-conditioned 4点でfitした
支配固有位相係数との相対差は全分割で0.288%以下だった。
...
`artifacts/pf_delta_validation/h4_sto3g_d100_rank12_ld{0,...,11}_v5.json` に保存する。
...
| 全local test（系サイズ検証追加後） | `444 passed, 4 warnings` |
