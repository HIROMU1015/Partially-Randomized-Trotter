#!/usr/bin/env python3
"""Synthesize the WP01-D/C07 compute result and review claim scope."""
...
from trotterlib.research_direction_decision_synthesis import (
    evaluate_wp01d_c07_synthesis,
    finalize_wp01d_synthesis_artifact,
    write_wp01d_synthesis_artifact,
)
...
def _provenance() -> dict[str, object]:
    sources = (
        Path("src/trotterlib/research_direction_decision_synthesis.py"),
        Path("scripts/run_research_direction_decision_synthesis.py"),
    )
    return {
...
def main() -> None:
    ...
    body = evaluate_wp01d_c07_synthesis(compute)
    ...
    artifact = finalize_wp01d_synthesis_artifact(
        body, provenance=_provenance()
    )
    write_wp01d_synthesis_artifact(artifact, args.output)
    print(
        json.dumps(
            {
