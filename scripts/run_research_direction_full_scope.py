#!/usr/bin/env python3
"""Run WP05-a full controlled-interrogation policy connection."""
...
from trotterlib.research_direction_ablation import validate_wp04_artifact
from trotterlib.research_direction_full_scope import (
    evaluate_wp05a_full_scope,
    finalize_wp05a_artifact,
    write_wp05a_artifact,
)
...
def _provenance() -> dict[str, object]:
    sources = (
        Path("src/trotterlib/df_partial_s2.py"),
        Path("src/trotterlib/df_partial_s2_repeated.py"),
        Path("src/trotterlib/research_direction_full_scope.py"),
        Path("scripts/run_research_direction_full_scope.py"),
    )
    return {
        ...
        "command": shlex.join(
            [
                ".venv311/bin/python",
                "scripts/run_research_direction_full_scope.py",
                *sys.argv[1:],
            ]
        ),
...
def main() -> None:
    ...
    body = evaluate_wp05a_full_scope(
        hamiltonian,
        preparations[3],
        preparations[12],
        compiler,
    ...
    artifact = finalize_wp05a_artifact(body, provenance=_provenance())
    write_wp05a_artifact(artifact, args.output)
    print(
        json.dumps(
            {
